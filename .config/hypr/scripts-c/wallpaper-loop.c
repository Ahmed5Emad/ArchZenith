...
bool get_wallpaper_for_workspace(const char* monitor, int workspace_id, char* wallpaper, size_t size) {
    ...
    if (!fp) {
        snprintf(config_path, sizeof(config_path), "%s/wallpaper-daemon/config/defaults.conf", hypr_dir);
    ...
    /* Fallback 1: Try workspace 1 if the specific workspace was not found */
    if (!found && workspace_id != 1) {
        fseek(fp, 0, SEEK_SET);
        strcpy(ws_key, "w-1=");
        while (fgets(line, sizeof(line), fp)) {
            line[strcspn(line, "\n")] = 0;
    ...
    /* Fallback 2: Try to find ANY first w- key in the file */
    if (!found) {
        fseek(fp, 0, SEEK_SET);
        while (fgets(line, sizeof(line), fp)) {
            line[strcspn(line, "\n")] = 0;
            if (strncmp(line, "w-", 2) == 0 && strchr(line, '=')) {
                char* val = strchr(line, '=') + 1;
                strncpy(wallpaper, val, size - 1);
                wallpaper[size - 1] = '\0';
                found = true;
                break;
            }
...
int main() {
    ...
    while (1) {
        ...
        if (sock < 0) {
            printf("Failed to connect to Hyprland event socket, retrying in 5 seconds...\n");
            sleep(5);
            continue;
        }
        ...
        fclose(sock_file);
        printf("Lost connection to Hyprland socket, reconnecting in 2 seconds...\n");
        sleep(2);
    }
