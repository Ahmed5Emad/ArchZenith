...
import Player, { downloadCover, getYoutubeThumbUrl } from "../../Player";
    ...
    <box class={"mpris"} spacing={5}>
      <For each={players}>
        {(player) => {
          ...
          const [coverPath, setCoverPath] = createState<string | null>(null);
          return (
            <menubutton>
              <overlay
                css={createComputed(() => {
                ...
                $={(self) => {
                  ...
                  self.connect("destroy", () => GLib.source_remove(timer));
                }}
              >
                <box />
                <Gtk.Picture
                  $type="overlay"
                  class="cover-art"
                  file={coverPath((p) => p ? Gio.File.new_for_path(p) : null)}
