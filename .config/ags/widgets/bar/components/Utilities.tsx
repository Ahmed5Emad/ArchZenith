...
function Battery() {
  ...
  return (
    <box
      ...
      class="battery"
    >
      <image pixelSize={11} iconName={createBinding(battery, "iconName")} />
      <label label={percent} />
    </box>
  );
}
...
export const activeTrayButtons = new Set<Gtk.MenuButton>();

export function refreshTrayMenus() {
  for (const btn of activeTrayButtons) {
    try {
      const model = btn.menuModel;
      btn.menuModel = null;
      btn.menuModel = model;
    } catch (e) {
      console.error("Error refreshing tray menu:", e);
    }
  }
}
...
function Tray() {
  ...
  const init = (btn: Gtk.MenuButton, item: AstalTray.TrayItem) => {
    try {
      activeTrayButtons.add(btn);
      btn.connect("destroy", () => {
        activeTrayButtons.delete(btn);
      });
      ...
      const updateMenu = () => {
        try {
          if (btn && typeof btn.insert_action_group === "function") {
            btn.menuModel = null;
            btn.menuModel = item.menuModel;
            if (item.actionGroup) {
              btn.insert_action_group("dbusmenu", item.actionGroup);
          ...
          console.error("Error updating system tray menu:", e);
        }
      };

      const actionGroupConnId = item.connect("notify::action-group", updateMenu);
      const menuModelConnId = item.connect("notify::menu-model", updateMenu);
      
      btn.connect("destroy", () => {
        try {
          if (item && typeof item.disconnect === "function") {
            if (actionGroupConnId) item.disconnect(actionGroupConnId);
            if (menuModelConnId) item.disconnect(menuModelConnId);
          }
        } catch (e) {
          console.error("Error disconnecting system tray notify signals:", e);
        }
      });
    } catch (e) {
      console.error("Error initializing system tray item:", e);
    }
  };
  ...
  return (
    <box
      ...
      $={(self) => {
        ...
        popover.add_css_class("resource-monitor-popover");
