import { Accessor, createState, createComputed } from "ags";
...
export default ({
  ...
  const [selectedIndex, setSelectedIndex] = createState<number>(0);
            ...
            {(result, index) => (
              <AppButton
                ...
                className={createComputed(() => {
                  return selectedIndex() === index() ? "checked" : "";
                })}
                onLaunch={onLaunch}
              />
            )}
          ...
          >
            <Gtk.EventControllerKey
              onKeyPressed={(
                ...
                const itemsCount = Results.get().length;
                ...
                if (itemsCount > 0 && currentIdx >= 0 && currentIdx < itemsCount) {
                  launchApp(Results.get()[currentIdx]);
                }
                return true;
              }}
            />
