...
export const connectPopoverEvents = (
  ...
  const tryConnect = () => {
    ...
    if (popover) {
      try {
        if (typeof (popover as any).set_has_arrow === "function") {
          (popover as any).set_has_arrow(false);
        }
      } catch (e) {
        print("Defensive warning: GJS set_has_arrow failed:", e);
      }
      try {
        if (typeof (popover as any).set_offset === "function") {
          (popover as any).set_offset(0, 8);
        }
      } catch (e) {
        print("Defensive warning: GJS set_offset failed:", e);
      }
