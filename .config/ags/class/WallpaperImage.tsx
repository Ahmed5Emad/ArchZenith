...
import { globalSettings, setGlobalSetting, focusedWorkspace } from "../variables";
...
let localFilesList: string[] = [];

function loadLocalFiles() {
  try {
    const cmd = `python3 -c "import os; home = os.path.expanduser('~'); dirs = [home + '/.config/wallpapers', home + '/Pictures/Wallpaper']; exts = {'png', 'jpg', 'jpeg', 'webp', 'gif', 'mp4'}; files = [os.path.join(r, f) for d in dirs if os.path.isdir(d) for r, _, fs in os.walk(d) for f in fs if f.lower().split('.')[-1] in exts]; print('\\n'.join(files))"`;
    const output = exec(cmd);
    localFilesList = output
      .trim()
      .split("\n")
      .filter((f) => f.length > 0 && !f.includes("/cache/thumbnails/"));
  } catch (e) {
    console.error("Failed to load local files for waifu widget:", e);
  }
}

// Initial load of local wallpapers/pictures
loadLocalFiles();
...
function getNextLocalImage(
  ...
  if (localFilesList.length === 0) loadLocalFiles();
  if (localFilesList.length === 0) return null;

  if (step === 0) {
    const randomIndex = Math.floor(Math.random() * localFilesList.length);
    return localFilesList[randomIndex];
  }

  const currentIndex = localFilesList.indexOf(currentPath);
  if (currentIndex === -1) {
    return localFilesList[0];
  }

  let nextIndex = (currentIndex + step) % localFilesList.length;
  if (nextIndex < 0) nextIndex = localFilesList.length - 1;
  return localFilesList[nextIndex];
}
...
type BookmarkActionResponse = {
  bookmarked?: boolean;
  bookmarks?: Array<Partial<WallpaperImage>>;
};

const parseBookmarkActionResponse = (raw: string): BookmarkActionResponse => {
  if (!raw?.trim()) {
    throw new Error("Received empty response from wallpaper bookmark action");
  }
...
/**
 * Unified WallpaperImage class
 *
 * This class serves as the single source of truth for:
 * - Image data and metadata
 * - Wallpaper metadata (tags, rating, source, etc.)
...
export class WallpaperImage {
   ...
   */
  getImagePath(): string {
    if (this.api.value === "local" || this.api.value === "custom") {
      return this.url || "";
    }
    return `${wallpaperPath}/${this.api.value}/images/${this.id}.${this.extension}`;
  }
  ...
  getPreviewPath(): string {
    ...
    return `${wallpaperPath}/${this.api.value}/previews/${this.id}.${this.extension}`;
  }
   ...
   *
   * Wallpaper preview files for video content are often JPEG stills with video
  ...
  isBookmarked(): boolean {
    ...
    if (WallpaperImage._bookmarkCacheHydrated) {
      this._isBookmarked = WallpaperImage._bookmarkKeys.has(key);
      return this._isBookmarked;
    }

    if (this._isBookmarked !== undefined) return this._isBookmarked;

    const currentBookmarks = globalSettings.peek().wallpaper.bookmarks;
    ...
    if (bookmarked) {
      WallpaperImage._bookmarkKeys.add(key);
    }
  ...
  static syncBookmarkCache(bookmarks: Array<Partial<WallpaperImage>>): void {
    ...
    WallpaperImage._bookmarkKeys = next;
    WallpaperImage._bookmarkCacheHydrated = true;
  }
  ...
  async fetchImage(): Promise<void> {
    ...
    try {
      ...
      await execAsync(["mkdir", "-p", `${wallpaperPath}/${this.api.value}/images`]);
      ...
      const curlArgs = ["curl", "-f", "-o", imagePath, imageUrl];
  ...
  async pinToTerminal(): Promise<void> {
    ...
    try {
      ...
      await execAsync(["bash", "-c", "pkill -SIGUSR1 fish || true"]);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
  ...
  async toggleBookmark(): Promise<boolean> {
    try {
      ...
      if (Array.isArray(parsed.bookmarks)) {
        WallpaperImage.syncBookmarkCache(parsed.bookmarks);
        setGlobalSetting("wallpaper.bookmarks", parsed.bookmarks);
      } else {
        WallpaperImage._bookmarkCacheHydrated = true;
        if (isBookmarked) {
          WallpaperImage._bookmarkKeys.add(this.getBookmarkKey());
          const currentBookmarks = globalSettings.peek().wallpaper.bookmarks;
          const exists = currentBookmarks.some(
            (img) => img.id === this.id && img.api.value === this.api.value,
          );
          if (!exists) {
            setGlobalSetting("wallpaper.bookmarks", [
              ...currentBookmarks,
              this.toJSON(),
            ]);
          }
        } else {
          WallpaperImage._bookmarkKeys.delete(this.getBookmarkKey());
          setGlobalSetting(
            "wallpaper.bookmarks",
            globalSettings
              .peek()
              .wallpaper.bookmarks.filter(
                (img) =>
   ...
   */
  static fromJSON(data: any): WallpaperImage {
    return new WallpaperImage(data);
  }

  /**
   * Fetch media by ID from wallpaper API
   *
   * @param id - The media ID to fetch
   * @param api - The wallpaper API to use
   * @returns Promise resolving to WallpaperImage instance with downloaded media
   */
  async fetchById(id: number, api: Api): Promise<WallpaperImage> {
    try {
      this._setLoadingState("loading");
      ...
      const res = await execAsync(
        `python ${GLib.get_home_dir()}/.config/ags/scripts/wallpaper.py ` +
          `--api ${api.value} ` +
          `--id ${id} `,
      );

      const imageData = readJson(res)[0];
      const image = new WallpaperImage({
        ...imageData,
        api: api,
      });
   ...
   *
   * Used in: WallpaperViewer grid items
  ...
  renderAsImageDialog(options?: {
    ...
    (popover as any).set_has_arrow(false);
    (popover as any).set_offset(0, 8);
    ...
    popover.connect("show", () => {
      ...
      const dialogContent = createRoot((dispose: () => void) => {
        ...
        const MediaDisplay = () => {
          ...
          if (this.isVideo()) {
            // Wallpaper video previews are always JPEG stubs — the real video only
  ...
  renderAsWaifuWidget(options?: {
    ...
    // Auto-initialize to local wallpaper if not already local or custom
    if (this.api.value !== "local" && this.api.value !== "custom") {
      if (localFilesList.length === 0) loadLocalFiles();
      if (localFilesList.length > 0) {
        const nextPath = localFilesList[Math.floor(Math.random() * localFilesList.length)];
        const ext = nextPath.split(".").pop() || "png";
        try {
          const sizeOut = exec(`identify -format "%h %w" ${JSON.stringify(nextPath)}`);
          const [height, width] = sizeOut.trim().split(" ");
          const localImage = new WallpaperImage({
            id: -2,
            height: Number(height) || 0,
            width: Number(width) || 0,
            api: { name: "Local", value: "local" } as any,
            extension: ext,
            url: nextPath,
            tags: ["local"],
          });
          ...
          GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
            setGlobalSetting("waifuWidget.current", localImage.toJSON());
            return false;
          });
        } catch (e) {
          console.error("Failed to parse dimensions for auto local wallpaper:", e);
        }
      }
    }
    ...
    // Actions component (Overlay with navigation and local tasks)
    const Actions = () => (
      <box
        class="actions"
      ...
      >
        ...
        {/* Row 1: Slideshow / Navigation */}
        <box class="section" spacing={5}>
          <button
            class="button prev-image"
            label=""
            tooltipMarkup="<b>Previous</b> local image"
            hexpand
            onClicked={() => {
              const currentPath = this.getImagePath();
              const nextPath = getNextLocalImage(currentPath, -1);
              if (nextPath) {
                const ext = nextPath.split(".").pop() || "png";
                try {
                  const sizeOut = exec(`identify -format "%h %w" ${JSON.stringify(nextPath)}`);
                  const [height, width] = sizeOut.trim().split(" ");
                  const nextImage = new WallpaperImage({
                    id: -2,
                    height: Number(height) || 0,
                    width: Number(width) || 0,
                    api: { name: "Local", value: "local" } as any,
                  ...
                  setGlobalSetting("waifuWidget.current", nextImage.toJSON());
                } catch (e) {
                  console.error("Failed to parse image size on prev:", e);
                }
              }
            }}
          />
          <button
            class="button random-image"
            label=""
            tooltipMarkup="<b>Random</b> local image"
            hexpand
            onClicked={() => {
              const currentPath = this.getImagePath();
              const nextPath = getNextLocalImage(currentPath, 0);
              if (nextPath) {
                const ext = nextPath.split(".").pop() || "png";
                try {
                  const sizeOut = exec(`identify -format "%h %w" ${JSON.stringify(nextPath)}`);
                  ...
                  console.error("Failed to parse image size on random:", e);
                }
              }
            }}
          />
          <button
            class="button next-image"
            label=""
            tooltipMarkup="<b>Next</b> local image"
            hexpand
            onClicked={() => {
              const currentPath = this.getImagePath();
              const nextPath = getNextLocalImage(currentPath, 1);
              if (nextPath) {
                ...
                try {
                  ...
                  console.error("Failed to parse image size on next:", e);
                }
              }
            }}
          />
        ...
        {/* Row 2: Set Wallpaper, Pin Terminal, Copy to Clipboard */}
        <box class="section" spacing={5}>
          <button
            class="button set-wallpaper"
            label=""
            tooltipMarkup="Set as <b>Desktop Wallpaper</b>"
            hexpand
            onClicked={(self) => {
              const currentWorkspaceId = focusedWorkspace.peek()?.id || 1;
              const monitorName = (self.get_root() as any).monitorName || "eDP-1";
              const imagePath = this.getImagePath();
              execAsync(
                `bash -c "$HOME/.config/hypr/wallpaper-daemon/set-wallpaper.sh ${currentWorkspaceId} ${monitorName} ${JSON.stringify(
                  imagePath,
                )}"`,
              )
                .then(() =>
                  notify({
                    ...
                    body: "Dynamic theme updated successfully!",
                  }),
                )
                .catch((err) =>
                  notify({ summary: "Error setting wallpaper", body: String(err) }),
                );
            }}
          />
          <togglebutton
          ...
          <button
            ...
            class="copy"
            sensitive={!this.isVideo()}
            tooltipMarkup="Copy image to clipboard"
            onClicked={() => this.copyToClipboard()}
          />
        </box>

        {/* Row 3: Swayimg / mpv External Viewer, CustomZen */}
        <box class="section" spacing={5}>
          <button
            label=" Viewer"
            class="open"
            hexpand
            tooltipMarkup="Open in <b>External Viewer</b>"
          ...
          <button
            ...
            hexpand
            class="upload"
            tooltipMarkup="Select custom image from <b>Disk</b>"
            onClicked={async (self) => {
              try {
                ...
                const customImage = new WallpaperImage({
                  id: -1,
                  height: Number(height) || 0,
                  width: Number(width) || 0,
...
export type Waifu = WallpaperImage;
