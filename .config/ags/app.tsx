...
import { refreshTrayMenus } from "./widgets/bar/components/Utilities";
...
app.start({
  ...
  requestHandler(argv: string[], response: (response: string) => void) {
    ...
    if (cmd == "delete-notification") {
      ...
      if (leftPanel) {
        leftPanel.show();
        const profileWidget = leftPanelWidgetSelectors.find((w) => w.name === "Profile");
        if (profileWidget) {
          setGlobalSetting("leftPanel.widget", profileWidget);
        } else {
          setGlobalSetting("leftPanel.widget", leftPanelWidgetSelectors[6]);
        }
      }
      response("Profile widget opened.");
      return;
      ...
      try {
        ...
        response(`Error reloading CSS: ${e}`);
      }
      return;
    }
